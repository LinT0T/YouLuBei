...
import com.youlubei.youlubei.utils.DateFactory;
...
public class GitHubContributionView extends View {
     ...
     * 灰色方格的默认颜色
     **/
    private final static int DEFAULT_BOX_COLOUR = 0xFFFFFFFF;
    /**
     * 提交次数颜色值
     **/
    ...
    private final static int[] DAY_OF_MONTH =
            new int[]{0, 31, 59, 87, 115, 150, 178, 206, 241, 269, 304, 332};
    /**
     * 星期
     **/
    ...
    public void initView() {
        mDays = DateFactory.getDays(2021, 5);
        //方格画笔
        boxPaint = new Paint();
        ...
        boxSide = Utils.dp2px(getContext(), boxSide);
...
//    }
    ...
    private void drawBox(Canvas canvas) {
        ...
        for (int i = DAY_OF_MONTH[month]; i < DAY_OF_MONTH[month + 2]; i++) {
            ...
            //给画笔设置当前天的颜色
            if (day.month != month + 1 && day.month != month + 2) {
                boxPaint.setColor(0XFFCCCCCC);
            } else {
                boxPaint.setColor(day.colour);
            }
            canvas.drawRect(startX, startY, endX, endY, boxPaint);
